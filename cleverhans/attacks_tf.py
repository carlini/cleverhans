...
class CarliniWagnerL0:
    def __init__(self, sess, model,
        ...
        initial_const: The initial tradeoff-constant to use to tune the
          relative importance of distance and confidence. Should be set to a
          very small value (but positive).
        largest_const: The largest constant to use until we report failure.
    ...
    def gradient_descent(self, sess, model):
        ...
        def doit(oimgs, labs, starts, valid, CONST):
            # re-scale images to be within range [0, 1]
            imgs = np.array(oimgs) - self.clip_min
            imgs /= self.clip_max - self.clip_min
    ...
    def attack(self, imgs, targets):
        ...
        If targeted is true, then the targets represents the target labels.
        If targeted is false, then targets are the original class labels.
        """
        r = []
